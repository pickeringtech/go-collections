...
// exact in T — it mirrors Sum, but with multiplication. For example the product
// of {2, 3, 4} is 24.
//
// The second return value is false — and the result the zero value of T — when
// input is empty, the empty product being undefined here by the (result, ok)
// convention rather than silently 1. Like Sum, Product is an exact-in-T
// reduction, so non-finite float inputs (NaN, ±Inf) propagate through the
// product per IEEE arithmetic and surface in the result with ok == true, rather
// than being rejected.
