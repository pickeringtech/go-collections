...
func (dl *DoublyLinked[T]) Filter(fn func(T) bool) List[T] {
...
func (dl *DoublyLinked[T]) RemoveAt(index int) List[T] {
	return NewArray(deleteOwned(dl.AsSlice(), index)...)
}
...
func (dl *DoublyLinked[T]) Remove(element T) List[T] {
	slice := dl.AsSlice()
...
func (dl *DoublyLinked[T]) Insert(index int, elements ...T) List[T] {
	slice := dl.AsSlice()
...
func (dl *DoublyLinked[T]) Sort(lessThan func(T, T) bool) List[T] {
	slice := dl.AsSlice()
...
func (dl *DoublyLinked[T]) SortInPlace(lessThan func(T, T) bool) {
	...
	sorted := dl.Sort(lessThan).AsSlice()
	dl.head = nil
	dl.tail = nil
	dl.size = 0
...
func (dl *DoublyLinked[T]) Push(element T) List[T] {
	slice := dl.AsSlice()
...
func (dl *DoublyLinked[T]) Pop() (T, bool, List[T]) {
	slice := dl.AsSlice()
	if len(slice) == 0 {
		var zero T
...
func (dl *DoublyLinked[T]) Enqueue(element T) List[T] {
...
func (dl *DoublyLinked[T]) Dequeue() (T, bool, List[T]) {
	slice := dl.AsSlice()
	if len(slice) == 0 {
		var zero T
		return zero, false, NewArray(slice...)
