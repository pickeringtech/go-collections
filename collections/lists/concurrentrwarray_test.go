...
func ExampleConcurrentRWArray_AsSlice() {
	arr := lists.NewConcurrentRWArray(1, 2, 3, 4, 5)
...
func TestConcurrentRWArray_AsSlice(t *testing.T) {
	type testCase[T any] struct {
		name string
		a    *lists.ConcurrentRWArray[T]
		want []T
...
func TestConcurrentRWArray_PopInPlace(t *testing.T) {
	...
	tests := []testCase[int]{
		...
		{
			name:     "empty ConcurrentRWArray yields zero value output and empty slice",
			a:        lists.NewConcurrentRWArray[int](),
			wantVal:  0,
			wantOK:   false,
			wantRest: []int{},
...
func TestConcurrentRWArray_Sort(t *testing.T) {
	...
	tests := []testCase[int]{
		...
		{
			name: "sorting empty ConcurrentRWArray results in empty slice",
			a:    lists.NewConcurrentRWArray[int](),
			args: args[int]{
				fn: slices.AscendingSortFunc[int],
			},
