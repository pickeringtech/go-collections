...
type EntryFunc[K comparable, V comparable] func(key K, value V)

// KeyValuesFunc is a function that operates on a key together with all of the
// values currently bound to it.
type KeyValuesFunc[K comparable, V comparable] func(key K, values []V)
...
type FilterFunc[K comparable, V comparable] func(key K, value V) bool
